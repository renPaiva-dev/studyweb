...
- **IDOR/controle de acesso**: RN01 aplicada nas rotas escopadas por usuário; unificação 403→404 para evitar enumeração, aplicada em `deck`/`colecao` (achado B15) e, desde `Docs/auditoria-coerencia-seguranca-2026-09.md`, também em `material`, `flashcard` e `quiz`/`tentativa` (achados C1/C2/C3/N7).
- **Timing oracle em login/esqueci-senha/reenviar-verificação** (achado C4, resolvido): os três fluxos rodam o mesmo custo de CPU (BCrypt) e de rede (envio de e-mail, para um destinatário descartado quando não há nada real a enviar) independente de o e-mail existir ou não — evita que a diferença de tempo de resposta reabra a enumeração que as mensagens genéricas (RN24/RN26) foram desenhadas para fechar.
- **Enumeração de conta no cadastro** (achado I1, resolvido): `POST /api/auth/cadastro` sempre responde `201`, mesmo quando o e-mail já está cadastrado — nesse caso nada é persistido e o dono real da conta recebe um e-mail avisando da tentativa, em vez do backend confirmar a existência da conta via `409`.
- **Headers de segurança do frontend** (achado N8, resolvido): `frontend/nginx.conf.template` configura CSP, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer` e `Permissions-Policy` — mesma camada de defesa em profundidade que já existia só do lado da API (`SecurityConfig.java`), agora também na página que o navegador de fato executa (onde o JWT fica em `localStorage`, sem `httpOnly`).
...
Nenhum gap confirmado em aberto no escopo desta auditoria (ver `Docs/auditoria-erros-2026-09.md` para o histórico completo, incluindo achados de baixo impacto tratados como melhoria opcional, não bug, e `Docs/auditoria-coerencia-seguranca-2026-09.md` para a auditoria de coerência produto/segurança de 2026-09-21).
